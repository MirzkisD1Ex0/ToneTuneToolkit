...
namespace ToneTuneToolkit.Networking.Upload
{
  ...
  {
    public static UnityAction<string> OnUploadFinishedString;
    public static UnityAction<Texture2D> OnUploadFinishedTexture2D;

    private const int AppID = 111;
    ...
    public void UpdateFilePackage(string projectN, string fileN, byte[] fileS)
    {
    ...
    #endregion
    // ==================================================
    #region Step 01 // 获取七牛token

    public void UploadFile() => StartCoroutine(GetQiniuAccessToken());
    private IEnumerator GetQiniuAccessToken()
    {
      using (UnityWebRequest unityWebRequest = UnityWebRequest.Get(QiniuAccessTokenURL))
      {
        ...
        else
        {
          ...
          StartCoroutine(PostFile2Qiniu()); // 下一步
        }
      }
    }
    ...
    private IEnumerator PostFile2Qiniu()
    {
      ...
      using (UnityWebRequest unityWebRequest = UnityWebRequest.Post(QiniuURL, wwwForm))
      {
        ...
        else
        {
          ...
          StartCoroutine(PostFile2Server()); // 下一步
        }
      }
    }
    ...
    private IEnumerator PostFile2Server()
    {
      ...
      using (UnityWebRequest unityWebRequest = new UnityWebRequest(cloudURL, "POST"))
      {
        ...
        else
        {
          ...
          // 返回链接
          OnUploadFinishedString?.Invoke(serverCallback.data.view_url);
          ...
          StartCoroutine(GetQRCode4Server()); // 下一步搞图
        }
      }
    }
    ...
    private IEnumerator GetQRCode4Server()
    {
      using (UnityWebRequest unityWebRequest = UnityWebRequestTexture.GetTexture(qrCodeURL)) // new UnityWebRequest(sunCodeURL, "GET"))
      {
        ...
        else
        {
          ...
          OnUploadFinishedTexture2D?.Invoke(t2dQRCode); // 返回图
        }
      }
    }
