...
namespace ToneTuneToolkit.IO.Printer
{
  ...
  public class PrinterManager : SingletonMaster<PrinterManager>
  {
    ...
    public void Print(Texture2D texture)
    {
      ...
      { Debug.LogWarning("[PM] 打印失败: " + e.Message); }
    }
      ...
      using (PrintDocument pd = new PrintDocument())
      {
        ...
        pd.PrintPage += (sender, args) =>
        {
          try
          {
            ...
            int paddingLeftRight = (int)(ps.Margin.x * 10); // 0.1英寸边距
            int paddingTopBottom = (int)(ps.Margin.y * 10); // 0.1英寸边距

            args.Graphics.Clear(System.Drawing.Color.White);

            Rectangle destRect = new Rectangle(
                paddingLeftRight,
                paddingTopBottom,
                args.PageBounds.Width - (paddingLeftRight * 2),
                args.PageBounds.Height - (paddingTopBottom * 2)
            );
