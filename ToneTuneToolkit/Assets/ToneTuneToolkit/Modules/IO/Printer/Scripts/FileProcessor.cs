...
namespace ToneTuneToolkit.IO.Printer
{
  ...
  {
    private const string PrinterPath = "C:/PrinterCache/";
    ...
    private void Init()
    {
      FolderCheck(PrinterPath);
      FolderCheck(CachePath);
      StartCoroutine(PrintLoop());
    }

    private void UnInit()
    {
      StopCoroutine(PrintLoop());
    }
    ...
    private void FolderCheck(string path)
    {
